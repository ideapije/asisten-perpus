...
$metakey_user=array(
	...
	,'kelas'	=>'Kelas'
	,'nohp'		=>'No. Hp Pribadi'
	,'alamatastr'=>'Alamat Astr'
	,'alamatdomisil'=>'Alamat domisil'); ?>
<h3>Tekan tombol Enter untuk menyimpan perubahan</h3>
<hr>
<form id="userakun" method="post" action="<?php echo esc_url(admin_url('admin-post.php'));?>">
<input type="hidden" value="update_profiluser" name="memberzone-penawaran-kirim" />
...
<?php foreach($metakey_user as $key => $value) : ?>
	<tr>
		...
		<td><?php
		if (cekosong(get_the_author_meta($key, $akun->ID))) :
			switch ($key) {
				case 'gender': ?>
				<select name="<?php echo $key;?>" id="txt-<?php echo $key;?>" class="regtablear-text">
					<option value="1">Laki-Laki</option>
					<option value="0">Perempuan</option>
				</select>
				<?php break;
				case 'nim': ?>
				<input type="number" name="<?php echo $key;?>" value="<?php echo get_the_author_meta($key, $akun->ID);?>"  id="txt-<?php echo $key;?>" class="regtablear-text input maxval" maxlength="8"/>
				<?php break; ?>
				<?php case 'nohp': ?>
				<input type="number" name="<?php echo $key;?>" value="<?php echo get_the_author_meta($key, $akun->ID);?>"  id="txt-<?php echo $key;?>" class="regtablear-text input"/>
				<?php break;
					default: ?>
					<input type="text" name="<?php echo $key;?>" value="<?php echo get_the_author_meta($key, $akun->ID);?>"  id="txt-<?php echo $key;?>" class="regtablear-text input"/>
				<?php break;
			}
		else: ?>
...
<span  id="fsubmit" style="display:none;">userakun</span>
